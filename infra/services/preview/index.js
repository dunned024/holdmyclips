...
const _replace_meta_tags = (body, clipId, clipDetails) => {
    ...
    body = _replace_meta_tag_content(body, "og:description", clipDetails["description"])
    body = _replace_meta_tag_content(body, "og:image", `/clips/${clipId}/${clipId}.png`)
    return body
}

const _replace_meta_tag_content = (body, tagName, replacementValue) => {
    const pattern = new RegExp(`<meta name="${tagName}" content="(.*?)"\/>`,"g")
    const replacer = replacementValue !== undefined 
