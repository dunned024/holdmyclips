...
export const handler = async (event, context, callback) => {
    ...
    if (event.httpMethod === "POST") {
        ...
        newCommentsObj = deleteCommentFromCommentsObject(comments, parsedData["commentId"])
...
const addCommentToCommentsObject = (comments, author, newCommentText) => {
    ...
    if (!comments.length) {
        ...
        nextId = Math.max.apply(Math, comments.map(function(o) { return o.commentId; })) + 1
    }
...
const deleteCommentFromCommentsObject = (comments, commentId) => {
    const newCommentsObj = {"comments": comments.filter(obj => obj.commentId != commentId)}
    return newCommentsObj
}
